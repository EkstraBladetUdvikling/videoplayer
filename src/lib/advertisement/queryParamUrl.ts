import { getKeyValues, getUserType } from 'frontend/js-admanager/src';
...
export async function getCustParamUrl(customParamInfo: TCustomParamInfo): Promise<string> {
  try {
    ...
    const { articleId, autoplayAllowed, inline, playerParent, sectionPath, videoType } = customParamInfo;
    ...
    if (ebBanners && playerParent) {
      const kvFromPath = getKeyValues ? [...getKeyValues(sectionPath, 'article'), articleId].join(',') : '';
      const userType = getUserType ? encodeURIComponent(getUserType(globalSegments)) : '';
