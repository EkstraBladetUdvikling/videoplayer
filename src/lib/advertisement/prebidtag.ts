...
import type { IDefaultAdvertisingReturn } from './types';
...
export async function getPrebidTag(
  ...
  advertisingOptions: IDefaultAdvertisingReturn
) {
  const extractedAdUnitName = tag.match(/iu=\/\d+\/ekstra_bladet\/(.*?)\/(.*?)&/);
  const adUnitName =
    extractedAdUnitName && extractedAdUnitName[2] ? extractedAdUnitName[2].replace('eb_', 'ekstra_bladet_') : '';
